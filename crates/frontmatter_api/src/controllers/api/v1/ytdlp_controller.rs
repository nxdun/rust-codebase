...
use shared_core::{
    error::AppError,
    models::{
        ytdlp::{YtdlpJobStatus, DownloadJob},
        ytdlp_dto::{
            YtdlpDownloadRequest, YtdlpJobResponse, YtdlpListResponse,
        },
    },
};
...
pub async fn create_download_job(
    ...
    let job_id = format!("ytdlp-{}", Uuid::new_v4());
    ...
    state.storage.push(DownloadJob {
        id: job_id.clone(),
        request: payload.clone(),
    }).await.map_err(|e| AppError::Internal(anyhow::anyhow!("Queue error: {e}")))?;

    let job_resp = YtdlpJobResponse {
        id: job_id,
        url: payload.url,
