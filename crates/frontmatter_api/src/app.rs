...
pub async fn run() {
    ...
    let db_pool = shared_core::db::postgres::init_db().await;
    let redis_pool = shared_core::db::redis::init_redis().await;

    // Initialize apalis storage
    let conn = apalis_redis::connect(config.redis_url.clone())
        .await
        .expect("Failed to connect to Redis for apalis");
